...
func BootstrapState(ctx context.Context, config *cfg.Config, dbProvider DBProvider, height uint64, appHash []byte) (err error) {
	...
	genState, stateStore, genDoc, err := LoadStateFromDBOrGenesisDocProvider(stateDB, DefaultGenesisDocProviderFunc(config), "")
	if err != nil {
		return err
	}
	...
	if appHash == nil {
		...
		if !bytes.Equal(appHash, genDoc.AppHash) {
			...
			return fmt.Errorf("the app hash returned by the light client does not match the provided appHash, expected %X, got %X", genDoc.AppHash, appHash)
		}
	}
...
func NewNodeWithContext(ctx context.Context,
	...
	// ====== =
	state, stateStore, genDoc, err := LoadStateFromDBOrGenesisDocProvider(stateDB, genesisDocProvider, "")
	// >>>>>>> 236b2fa730 (refactor: backport remove genesis persistence from state)
	if err != nil {
		return nil, err
	}
	...
	//
	// || || || | parent of 236b2fa730 (refactor: backport remove genesis persistence from state)
	// ====== =
	...
	// to fail and prevent the node from booting.--5
	if exists, _ := stateDB.Has(genesisDocKey); exists {
...
func LoadStateFromDBOrGenesisDocProvider(
...
) (sm.State, sm.BootstrapStore, *types.GenesisDoc, error) {
	// 1. Verify genesisDoc hash in db if exists
	genDocHash, err := stateDB.Get(genesisDocHashKey)
	if err != nil {
		return sm.State{}, sm.BootstrapStore{}, nil, ErrRetrieveGenesisDocHash{Err: err}
	}

	csGenDoc, err := genesisDocProvider()
	if err != nil {
		return sm.State{}, sm.BootstrapStore{}, nil, err
	}

	if err = csGenDoc.GenesisDoc.ValidateAndComplete(); err != nil {
		return sm.State{}, sm.BootstrapStore{}, nil, ErrGenesisDoc{Err: err}
	}
	...
	if err := tmhash.ValidateSHA256(checkSumStr); err != nil {
		...
		return sm.State{}, sm.BootstrapStore{}, nil, fmt.Errorf(formatStr, err)
	}
	...
	if operatorGenesisHashHex != "" {
		...
		if err != nil {
			return sm.State{}, sm.BootstrapStore{}, nil, ErrGenesisHashDecode
		}
		if !bytes.Equal(csGenDoc.Sha256Checksum, decodedOperatorGenesisHash) {
			return sm.State{}, sm.BootstrapStore{}, nil, ErrPassedGenesisHashMismatch
		}
	}
	...
	if len(genDocHash) == 0 {
		...
		if err = stateDB.SetSync(genesisDocHashKey, csGenDoc.Sha256Checksum); err != nil {
			return sm.State{}, sm.BootstrapStore{}, nil, ErrSaveGenesisDocHash{Err: err}
		}
	} else {
		if !bytes.Equal(genDocHash, csGenDoc.Sha256Checksum) {
			return sm.State{}, sm.BootstrapStore{}, nil, ErrLoadedGenesisDocHashMismatch
		}
	}
